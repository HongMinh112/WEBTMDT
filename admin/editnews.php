...
if($_GET['ID']){
    $ID=$_GET['ID'];
    $update=mysqli_query($conn,"SELECT * FROM news WHERE ID=$ID");
    $query=mysqli_fetch_assoc($update);
}
?>
<!DOCTYPE html>
<html lang="en">
            ...
            <div class="catogery_manenger" >
                ...
                <form action="../controler/admin/news.php" method="post" enctype="multipart/form-data">
                    ...
                    <div style="margin-bottom: 10px;margin-top: 20px;">
                        ...
                        <input type="text" name="name" placeholder="Tiêu đề..." value="<?php echo $query['nameEvent'] ?>">
                    </div>
                    <div style="margin-left: 55px;margin-bottom: 10px;">
                        <label for="">Hình Ảnh: </label>
                        <input id="check" type="file" id="image1" name="image"  placeholder="Hình Ảnh...">
                        <img src="../Content/Img/<?php echo $query['image'] ?>" alt="" width="120px" height="120px">
                    </div>
                    <div style="margin-bottom: 10px;margin-top: 20px; display:flex;margin-left: 155px;">
                        ...
                        <textarea name="content" id="" cols="30" rows="7" placeholder="Nội dung..." ><?php echo $query['content'] ?></textarea>
                    </div>
                    <div style="margin-left: -130px;margin-bottom: 10px;">
                        <label for="">Trang Thai</label>
                        <label style="width:30px;" for="">
                            <input style="width:30px;" type="radio" name="status" value="0" <?php echo ($query['status']==0?'checked':'') ?>>Ẩn
                        </label>
                       <label for="">
                           <input style="width:30px;" type="radio" name="status" value="1"<?php echo ($query['status']==1?'checked':'')?>>Hiện
                       </label>
                    </div>
                    <div style="margin-top: 10px;margin-bottom: 10px;">
                        <input type="hidden" name="updatenews" value="update" style="border-radius: 5px;">
                        <button>Thêm</button>
                    </div>
                </form>
            </div>
